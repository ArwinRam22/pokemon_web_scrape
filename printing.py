...
def print_basic_info(starting_pokemon, vitals, forms):
    ...
    for vital in vitals:
        if (iterations % num_tables == 0) or (iterations == 0):
            #if Pokemon has no other forms (eg. Bulbasaur, Charmander, etc.)
            if len(forms)-2 == 1:
                print('<'*25 + f' {starting_pokemon.upper()} ' + '>'*25)
             #if Pokemon has other forms (eg. Zacian, Meowth, etc.)
            else:
        ...
        for row in rows:
            ...
            if header == 'Local №':
                ...
                if header == 'Type':
                    data = reformat.reformat_type(data)
                elif header == 'Abilities':
                    data = reformat.reformat_ability(data.replace(' (hidden ability)', '*'))
...
def print_move(moves, header):
    ...
    for move in moves:
        ...
        if header not in ('Egg moves', 'Move Tutor moves', 'Transfer-only moves', 'Pre-evolution moves'):
